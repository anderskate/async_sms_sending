...
async def main():
    ...
    try:
        ...
        add_sms_mailing = partial(db.add_sms_mailing, sms_id, phones, text)
        await trio_asyncio.asyncio_as_trio(add_sms_mailing)

        sms_ids = await trio_asyncio.asyncio_as_trio(db.list_sms_mailings())
        print('Registered mailings ids', sms_ids)

        pending_sms_list = await trio_asyncio.asyncio_as_trio(
        ...
        sms_mailings = await trio_asyncio.asyncio_as_trio(
        ...
        async def send():
            while True:
                await trio.sleep(1)
                await trio_asyncio.asyncio_as_trio(
                    redis.publish('updates', sms_id)
                )

        async def listen():
            channel = redis.pubsub()
            await trio_asyncio.asyncio_as_trio(channel.subscribe('updates'))

            while True:
                message = await trio_asyncio.asyncio_as_trio(
                    channel.get_message(
        ...
        await trio_asyncio.asyncio_as_trio(redis.close())
